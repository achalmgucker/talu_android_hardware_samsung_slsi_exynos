...
static int gralloc_unmap(gralloc_module_t const* module __unused, buffer_handle_t handle)
{
    ...
    case HAL_PIXEL_FORMAT_EXYNOS_YCbCr_420_SP_M_PRIV:
        ...
        if (munmap(INT_TO_PTR(hnd->base2), PRIV_SIZE) < 0) {
            ALOGE("%s: could not unmap %s %llx %zu", __func__, strerror(errno), (unsigned long long)hnd->base2, chroma_size);
        }
        hnd->base2 = 0;
        break;
    default:
    ...
    if (munmap(INT_TO_PTR(hnd->base), hnd->size) < 0) {
        ALOGE("%s: could not unmap %s %llx %d", __func__, strerror(errno),
              (unsigned long long)hnd->base, hnd->size);
    }
    ALOGV("%s: base %llx %d %d %d %d\n", __func__, (unsigned long long)hnd->base,
    ...
    if (hnd->fd1 >= 0) {
        ...
        if (munmap(INT_TO_PTR(hnd->base1), chroma_size) < 0) {
            ALOGE("%s: could not unmap %s %llx %zu", __func__, strerror(errno),
                  (unsigned long long)hnd->base1, chroma_size);
        }
        hnd->base1 = 0;
    }
    if (hnd->fd2 >= 0) {
        ...
        if (munmap(INT_TO_PTR(hnd->base2), chroma_size) < 0) {
            ALOGE("%s: could not unmap %s %llx %zu", __func__, strerror(errno),
                  (unsigned long long)hnd->base2, chroma_size);
        }
        hnd->base2 = 0;
    }
    return 0;
